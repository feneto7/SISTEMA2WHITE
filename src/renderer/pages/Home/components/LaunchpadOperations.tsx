//--------------------------------------------------------------------
// LAUNCHPAD - Vista de grade de ícones estilo macOS
// Componente que exibe os módulos de operações comerciais
...
interface LaunchpadOperationsProps {
  isOpen: boolean;
  onClose: () => void;
...
export function LaunchpadOperations({ isOpen, onClose, onSelectOperation }: LaunchpadOperationsProps): JSX.Element | null {
  ...
  React.useEffect(() => {
    ...
    `;
    if (!document.head.querySelector('style[data-launchpad]')) {
      style.setAttribute('data-launchpad', 'true');
      document.head.appendChild(style);
    }
    return () => {
      const existingStyle = document.head.querySelector('style[data-launchpad]');
      if (existingStyle) {
